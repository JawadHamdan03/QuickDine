...
const getCurrentUserFromToken = async (req: Request) => {
...
export const getRestaurants = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        const queryObj: Record<string, any> = { status: "approved" };
        ...
        if (priceRange) {
            ...
            const validPrices = prices.filter((item): item is string => typeof item === "string" && item.trim().length > 0);
...
export const getRestaurantBySlug = async (req: AuthRequest, res: Response): Promise<void> => {
    try {
